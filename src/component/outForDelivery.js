...
export const OutForDelivery = ({getDelivereryData}) => {
    const [data,setData] = useState([]);
    const [loading,setLoading] = useState(false);
    const getData = () => {
        setLoading(true);
        axios.get(`https://0q54sk5m7j.execute-api.us-east-2.amazonaws.com/dev/admin/order/get?status=delivery`,{withCredentials:true})
        .then(res=>{
            if(res.status===200){
                setData(res.data);
                setLoading(false);
